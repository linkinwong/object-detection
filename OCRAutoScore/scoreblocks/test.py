...
class Model:
    ...
    def detect_answers_with_yolo(self, img_path, conf_threshold=0.25):
        ...
        for result in results:
            if result.boxes is not None:
                for i in range(len(result.boxes)):
                    ...
                    if box_conf >= conf_threshold and box_cls == 3:  # 假设3类是objective_problem
...
if __name__ == "__main__":
    ...
    img_path = "/Users/pingzhang/AI/OCRAutoScore/test.png" #input("请输入图片路径: ")
    correct_answers = "B,D,A,C,C,D,C,A,A,C,B,A,D,D,C,D,C,A,D,C".split(",") #input("请输入正确答案(多个答案请用逗号分隔): ").split(",")
    img = cv2.imread(img_path)
    # img = model.scale_image(img) # type: ignore
    boxes = model.detect_answers_with_yolo(img, conf_threshold=0.25)  # 设置置信度阈值
    if not boxes:
        print("未检测到答题框")
        # continue
