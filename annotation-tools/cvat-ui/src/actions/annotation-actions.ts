...
export enum AnnotationActionTypes {
    ...
    AI_TOOLS_AUTO_ANNOTATION = 'AI_TOOLS_AUTO_ANNOTATION',
    AI_TOOLS_LOADING = 'AI_TOOLS_LOADING',
    ...
    return async (dispatch: ActionCreator<Dispatch>, getState): Promise<void> => {
        ...
        try {
            ...
            for (let frame = from + sign; sign > 0 ? frame <= to : frame >= to; frame += sign) {
                copy.frame = frame;
                copy.elements.forEach((element: any) => {
                    element.frame = frame;
                });
...
export function changeFrameAsync(
    ...
    return async (dispatch: ActionCreator<Dispatch>, getState: () => CombinedState): Promise<void> => {
        ...
        try {
            if (job && (toFrame < job.startFrame || toFrame > job.stopFrame)) {
...
export function getJobAsync({
    ...
    return async (dispatch: ActionCreator<Dispatch>, getState): Promise<void> => {
        try {
            ...
            try {
                if (['text', 'audio'].indexOf(job.taskType) === -1) {
            ...
            const textRequests = async () => {
                let ret = [];
                await Axios.get(`/text/get_task?id=${task.id}`).then((response) => {
                    if (!response.data?.task) return null;
                    for (const task of response.data.task) {
                        const dataUrl = `/text${task.url}`;
                        let textName = task.url.split('/');
            ...
            if (['text'].includes(task.type)) {
                job.otherTypeData = await textRequests();
            }
...
export function AIToolsAutoAnnotation(states): AnyAction {
    return {
        type: AnnotationActionTypes.AI_TOOLS_AUTO_ANNOTATION,
        payload: {
            states,
        },
    };
...
export function AIToolsLoading(loading): AnyAction {
    return {
        type: AnnotationActionTypes.AI_TOOLS_LOADING,
        payload: {
            loading,
        },
    };
...
export function GetAudioInfo(state: any): AnyAction {
    return {
        type: AnnotationActionTypes.GET_AUDIO_INFO,
        payload: {
            state,
        },
    };
