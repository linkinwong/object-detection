...
AnnotationStages.prototype = {
    ...
    getAnnotationData: function(region) {
        var regionData = {
            ...
            label: 'test',
            content: $(region.element).children('div').eq(1).text(),
            // 'annotation': region.annotation
            // annotation: [$(region.element).children('div').eq(1).text()],
            // title: $(region.element).attr('title'),
        };
    ...
    getAnnotations: function() {
        ...
        if (this.wavesurfer.regions) {
            for (var region_id in $.extend({}, this.wavesurfer.regions.list)) {
                var region = this.wavesurfer.regions.list[region_id];
                if (region.element.className === 'wavesurfer-region-audio-finish') {
                    annotationData.push(this.getAnnotationData(region));
                } else {
                    delete this.wavesurfer.regions.list[region_id];
                }
            }
        }
    ...
    reset: function(proximityTags, annotationTags, solution, alwaysShowTags) {
        ...
        // Update solution set
        this.annotationSolutions = solution || [];
