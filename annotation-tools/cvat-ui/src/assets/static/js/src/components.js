...
PlayBar.prototype = {
    ...
    addWaveSurferEvents: function() {
        ...
        this.wavesurfer.on('play', function () {
            $('.play_audio').removeClass('fa-play-circle').addClass('fa-pause');
        });

        this.wavesurfer.on('pause', function () {
            $('.play_audio').removeClass('fa-pause').addClass('fa-play-circle');
        });
...
WorkflowBtns.prototype = {
    ...
    createLabels: function() {
        var my = this;
        this.labelBtn = $('<button>', {
            class: 'btn submit waves-effect',
            text: '确认标签'
        });
        this.labelBtn.click(function () {
            $(my).trigger('save-labels');
        });
    },

    // Append the next and exit elements to the the parent container
    update: function() {
        $('.job-select').append(this.labelBtn, this.saveBtn);
    },
