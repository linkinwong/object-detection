...
function ActionsMenuComponent(props: Props): JSX.Element {
    ...
    menuItems.push([(
        <Menu.Item key={Actions.LOAD_TASK_ANNO}>
            {t('actions-menu_actions-menu_tsx.Uploadannotations', 'Upload annotations')}
        </Menu.Item>
    ), 0]);

    menuItems.push([(
        <Menu.Item key={Actions.EXPORT_TASK_DATASET}>
            {t('actions-menu_actions-menu_tsx.Exporttaskdataset', 'Export task dataset')}
        </Menu.Item>
    ), 10]);

    if (bugTracker) {
        menuItems.push([(
            <Menu.Item key={Actions.OPEN_BUG_TRACKER}>
                {t('actions-menu_actions-menu_tsx.Openbugtracker', 'Open bug tracker')}
            </Menu.Item>
        ), 20]);
    }
    ...
    menuItems.push([(
        <Menu.Item disabled={inferenceIsActive} key={Actions.RUN_AUTO_ANNOTATION}>
            {t('actions-menu_actions-menu_tsx.Automaticannotation', 'Automatic annotation')}
        </Menu.Item>
    ), 30]);
    ...
    menuItems.push([(
        ...
        >
            {t('actions-menu_actions-menu_tsx.BackupTask', 'Backup Task')}
        </Menu.Item>
    ), 40]);
    ...
    menuItems.push([(
        ...
        >
            {t('actions-menu_actions-menu_tsx.Viewanalytics', 'View analytics')}
        </Menu.Item>
    ), 50]);

    if (projectID === null) {
        menuItems.push([(
            <Menu.Item key={Actions.MOVE_TASK_TO_PROJECT}>
                {t('actions-menu_actions-menu_tsx.Movetoproject', 'Move to project')}
            </Menu.Item>
        ), 60]);
    }
    ...
    menuItems.push([(
        <React.Fragment key={Actions.DELETE_TASK}>
            <Menu.Divider />
            <Menu.Item key={Actions.DELETE_TASK}> 
                {t('actions-menu_actions-menu_tsx.Delete', 'Delete')}
            </Menu.Item>
        </React.Fragment>
    ), 70]);
