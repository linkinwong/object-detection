        ...
        <div className='cvat-brush-tools-toolbox' style={{ top, left, display: visible ? '' : 'none' }}>
            ...
            { ['brush', 'eraser'].includes(currentTool) ? (
                <Select value={brushForm} onChange={(value: 'circle' | 'square') => setBrushForm(value)}>
                    <Select.Option value='circle'>{t('annotation-page_canvas_views_canvas2d_brush-tools_tsx.Circle', 'Circle')}</Select.Option>
                    <Select.Option value='square'>{t('annotation-page_canvas_views_canvas2d_brush-tools_tsx.Square', 'Square')}</Select.Option>
                </Select>
            ) : null}
            <Button
                type='text'
