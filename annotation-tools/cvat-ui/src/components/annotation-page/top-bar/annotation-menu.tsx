...
function AnnotationMenuComponent(props: Props & RouteComponentProps): JSX.Element {
        ...
        if (params.key === Actions.REMOVE_ANNOTATIONS) {
            ...
            Modal.confirm({
                ...
                content: (
                    <div>
                        <Text>
                            {t('annotation-page_top-bar_annotation-menu_tsx.Youaregoingtoremovet', 'You are going to remove the annotations from the client. ')}
                        </Text>
                        <Text>
                            {t('annotation-page_top-bar_annotation-menu_tsx.Itwillstayontheserve', 'It will stay on the server till you save the job. Continue?')}
                        </Text>
                        <br />
                        <br />
                        <Collapse bordered={false}>
                            <Panel header={<Text>{t('annotation-page_top-bar_annotation-menu_tsx.SelectRange', 'Select Range')}</Text>} key={1}>
                                <Text>From: </Text>
                                <InputNumber
                                    min={0}
                                    max={stopFrame}
                                ...
                                <CVATTooltip title='Applicable only for annotations in range'>
                                    ...
                                    >
                                        {t('annotation-page_top-bar_annotation-menu_tsx.Deleteonlykeyframesf', 'Delete only keyframes for tracks')}
                                    </Checkbox>
                                </CVATTooltip>
                            </Panel>
                        </Collapse>
    ...
    return (
        <Menu
            ...
            selectable={false}
        >
            <Menu.Item key={Actions.LOAD_JOB_ANNO}>{t('annotation-page_top-bar_annotation-menu_tsx.Uploadannotations', 'Upload annotations')}</Menu.Item>
            <Menu.Item key={Actions.EXPORT_JOB_DATASET}>{t('annotation-page_top-bar_annotation-menu_tsx.Exportjobdataset', 'Export job dataset')}</Menu.Item>
            <Menu.Item key={Actions.REMOVE_ANNOTATIONS}>{t('annotation-page_top-bar_annotation-menu_tsx.Removeannotations', 'Remove annotations')}</Menu.Item>
            <Menu.Item
            ...
            >
                {t('annotation-page_top-bar_annotation-menu_tsx.Runactions', 'Run actions')}
            </Menu.Item>
            <Menu.Item key={Actions.OPEN_TASK}>
                ...
                >
                    {t('annotation-page_top-bar_annotation-menu_tsx.Openthetask', 'Open the task')}
                </a>
            </Menu.Item>
            ...
            {[JobStage.ANNOTATION, JobStage.VALIDATION].includes(jobStage) ?
                <Menu.Item key={Actions.FINISH_JOB}>{t('annotation-page_top-bar_annotation-menu_tsx.Finishthejob', 'Finish the job')}</Menu.Item> : null}
            {jobStage === JobStage.ACCEPTANCE ?
                <Menu.Item key={Actions.RENEW_JOB}>{t('annotation-page_top-bar_annotation-menu_tsx.Renewthejob', 'Renew the job')}</Menu.Item> : null}
        </Menu>
    );
}
