...
class CreateTaskContent extends React.PureComponent<Props & RouteComponentProps, State> {
    ...
    private supportedFileFormats: any;
    ...
    public constructor(props: Props & RouteComponentProps) {
        ...
        this.supportedFileFormats = {
            ...
            audio: ['mp3', 'wav'],
            text: ['mp3', 'wav', 'txt', 'doc', 'docx', 'ppt', 'html'],
            video: ['mp4', 'avi', 'wmv'],
        };
    }
    ...
    private handleUploadLocalFiles = (uploadedFiles: File[]): void => {
        ...
        const type = this.basicConfigurationComponent.current.formRef.current.getFieldValue('type');
        if (!type) {
            uploadFileErrorMessage = '请先选择任务类型';
        } else {
            const fileFormats = this.supportedFileFormats[type];
            const excludedManifests = uploadedFiles.filter(
                (x: File) => {
                    for (const fileFormat of fileFormats) {
                        if (x.name.endsWith('.' + fileFormat) ||
                            x.name.endsWith('.' + fileFormat.toUpperCase())) {
                            return false;
                        }
                    }
                    return true;
            ...
            if (excludedManifests.length > 0) {
                uploadFileErrorMessage = `${UploadFileErrorMessages.shinemo}, \(${fileFormats}\). `;
            }

            // if (!many && excludedManifests.length > 0) {
    ...
    private validateBlocks = (): Promise<any> => new Promise((resolve, reject) => {
        ...
        if (!this.validateLabelsOrProject()) {
            notification.error({
                message: '无法创建任务',
                description: '任务必须包含至少一个标签或属于某个项目',
    ...
    private handleSubmitMultiTasks = (): void => {
        this.validateBlocks()
            ...
            .then(() => {
                ...
                notification.info({
                    message: useTranslation().t('create-task-page_create-task-content_tsx.SubmitMultiTasksMsg'),
                    description:
