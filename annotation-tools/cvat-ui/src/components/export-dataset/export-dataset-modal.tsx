...
function ExportDatasetModal(props: StateToProps): JSX.Element {
    ...
    const handleExport = useCallback(
        (values: FormValues): void => {
            ...
            closeModal();
            const resource = values.saveImages ? t('export-dataset_export-dataset-modal_tsx.Dataset') : t('export-dataset_export-dataset-modal_tsx.Annotations');
            Notification.info({
                message: `${resource} ${t('export-dataset_export-dataset-modal_tsx.exportStarted')}`,
                description:
                    `${resource} ${t('export-dataset_export-dataset-modal_tsx.ExportMsg')} ${instanceType}. ` +
                    `资源准备好，下载将自动开始`,
    ...
    return (
        <Modal
            title={<Text strong>{`导出 ${instanceType} 作为一个数据集`}</Text>}
            visible={!!instance}
            onCancel={closeModal}
            onOk={() => form.submit()}
        ...
        >
            ...
            >
                <Form.Item
                    name='selectedFormat'
                    label={<Text strong>{t('export-dataset_export-dataset-modal_tsx.Exportformat', 'Export format')}</Text>}
                ...
                <Space>
                    ...
                    <Text strong>{t('export-dataset_export-dataset-modal_tsx.Saveimages', 'Save images')}</Text>
                </Space>

                <Form.Item label={<Text strong>{t('export-dataset_export-dataset-modal_tsx.Customname', 'Custom name')}</Text>} name='customName'>
                    <Input
                        placeholder={t('export-dataset_export-dataset-modal_tsx.Customnameplaceholder')}
                        suffix='.zip'
                ...
                <TargetStorageField
                    ...
                    switchDescription={t('export-dataset_export-dataset-modal_tsx.targetStorageDescription')}
                    switchHelpMessage={helpMessage}
                    useDefaultStorage={useDefaultTargetStorage}
                    storageDescription={t('export-dataset_export-dataset-modal_tsx.TargetStorageStorageDescription')}
