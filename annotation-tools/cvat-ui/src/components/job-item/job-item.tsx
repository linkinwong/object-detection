    ...
    if (!summary) {
        if (error) {
            if (error.toString().includes('403')) {
                return <p>{t('job-item_job-item_tsx.Youdonothavepermissi', 'You do not have permissions')}</p>;
            }

            return <p>{t('job-item_job-item_tsx.Couldnotfetchcheckco', 'Could not fetch, check console output')}</p>;
        }
    ...
    return (
        <table className='cvat-review-summary-description'>
            <tbody>
                <tr>
                    <td>
                        <Text strong>{t('job-item_job-item_tsx.Unsolvedissues', 'Unsolved issues')}</Text>
                    </td>
                    <td>{summary.issues_unsolved}</td>
                </tr>
                <tr>
                    <td>
                        <Text strong>{t('job-item_job-item_tsx.Resolvedissues', 'Resolved issues')}</Text>
                    </td>
            ...
            <Card className='cvat-job-item' style={{ ...style }} data-row-id={job.id}>
                <Row>
                    <Col span={7}>
                        <Row>
                            <Col>
                                <Link to={`/jobs/${job.taskId}/${job.id}`}>{`作业 #${job.id}`}</Link>
                            </Col>
                            {
                                job.type === JobType.GROUND_TRUTH && (
                                    <Col offset={1}>
                                        <Tag color='#ED9C00'>{t('job-item_job-item_tsx.Groundtruth', 'Ground truth')}</Tag>
                                    </Col>
                                )
                            }
                        </Row>
                        <Row className='cvat-job-item-dates-info'>
                            <Col>
                                <Text>{t('job-item_job-item_tsx.Createdon', 'Created on')} </Text>
                                <Text type='secondary'>{`${created.format('YYYY MM DD HH:mm')}`}</Text>
                            </Col>
                        </Row>
                        <Row>
                            <Col>
                                <Text>{t('job-item_job-item_tsx.Lastupdated', 'Last updated')} </Text>
                            ...
                            <Col>
                                <Row>
                                    <Col className='cvat-job-item-select'>
                                        <Row>
                                            <Text>{t('job-item_job-item_tsx.Assignee', 'Assignee:')}</Text>
                                        </Row>
                                        <UserSelector
                                            ...
                                            onSelect={(user: User | null): void => {
                                                ...
                                                job.assignee = user;
                                                job.assigneeUrl = `${window.location.origin}/tasks/${job.taskId}/jobs/${job.id}`;
                                                onJobUpdate(job);
                                            }}
                                        />
                                    </Col>
                                    <Col className='cvat-job-item-select'>
                                        <Row justify='space-between' align='middle'>
                                            <Col>
                                                <Text>{t('job-item_job-item_tsx.Stage', 'Stage:')}</Text>
                                            </Col>
                                            <Col>
                        ...
                        <Row className='cvat-job-item-details'>
                            <Col>
                                <Row>
                                    <Col>
                                        <ProjectOutlined />
                                        <Text>{t('job-item_job-item_tsx.State', 'State:')} </Text>
                                ...
                                <Row>
                                    <Col>
                                        ...
                                        <Text>{t('job-item_job-item_tsx.Duration', 'Duration:')} </Text>
                            ...
                            <Col offset={2}>
                                    ...
                                    job.type !== JobType.GROUND_TRUTH && (
                                        <Row>
                                            <Col>
                                                ...
                                                <Text>{t('job-item_job-item_tsx.Framerange', 'Frame range:')} </Text>
                                ...
                                <Row>
                                    <Col>
                                        <BorderOutlined />
                                        <Text>{t('job-item_job-item_tsx.Framecount', 'Frame count:')} </Text>
