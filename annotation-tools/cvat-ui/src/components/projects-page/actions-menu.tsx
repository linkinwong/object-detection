...
function ProjectActionsMenuComponent(props: Props): JSX.Element {
    ...
    menuItems.push([(
        <Menu.Item key='export-dataset' onClick={() => dispatch(exportActions.openExportDatasetModal(projectInstance))}>
            {t('projects-page_actions-menu_tsx.Exportdataset', 'Export dataset')}
        </Menu.Item>
    ), 0]);

    menuItems.push([(
        <Menu.Item key='import-dataset' onClick={() => dispatch(importActions.openImportDatasetModal(projectInstance))}>
            {t('projects-page_actions-menu_tsx.Importdataset', 'Import dataset')}
        </Menu.Item>
    ), 10]);
    ...
    menuItems.push([(
        ...
        >
            {t('projects-page_actions-menu_tsx.BackupProject', 'Backup Project')}
        </Menu.Item>
    ), 20]);
    ...
    menuItems.push([(
        <Menu.Item key='view-analytics'>
            ...
            >
                {t('projects-page_actions-menu_tsx.Viewanalytics', 'View analytics')}
            </a>
        </Menu.Item>
    ), 30]);
    ...
    menuItems.push([(
        <Menu.Item key='set-webhooks'>
            ...
            >
                {t('projects-page_actions-menu_tsx.Setupwebhooks', 'Setup webhooks')}
            </a>
        </Menu.Item>
    ), 40]);
    ...
    menuItems.push([(
        <React.Fragment key='delete'>
            ...
            <Menu.Item key='delete' onClick={onDeleteProject}>
                {t('projects-page_actions-menu_tsx.Delete', 'Delete')}
            </Menu.Item>
        </React.Fragment>
    ), 50]);
