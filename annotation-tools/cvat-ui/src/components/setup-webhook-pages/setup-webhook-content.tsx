...
function SetupWebhookContent(props: Props): JSX.Element {
    ...
    return (
        <Row justify='start' align='middle' className='cvat-setup-webhook-content'>
            <Col span={24}>
                <Text className='cvat-title'>{t('setup-webhook-pages_setup-webhook-content_tsx.Setupawebhook', 'Setup a webhook')}</Text>
            </Col>
            <Col span={24}>
                <Form
                    form={form}
                ...
                >
                        ...
                        !webhook && (
                            <Row className='ant-form-item'>
                                <Col className='ant-form-item-label' span={24}>
                                    <Text className='cvat-text-color'>{t('setup-webhook-pages_setup-webhook-content_tsx.Project', 'Project')}</Text>
                                </Col>
                                <Col span={24}>
                                    <ProjectSearchField
                    ...
                    >
                        <Checkbox>
                            <Text className='cvat-text-color'>{t('setup-webhook-pages_setup-webhook-content_tsx.EnableSSL', 'Enable SSL')}</Text>
                        </Checkbox>
                    </Form.Item>
                    <Form.Item
                    ...
                    >
                        <Checkbox>
                            <Text className='cvat-text-color'>{t('setup-webhook-pages_setup-webhook-content_tsx.Active', 'Active')}</Text>
                        </Checkbox>
                    </Form.Item>
                    <Form.Item
                        name='eventsMethod'
                    ...
                    >
                        <Radio.Group onChange={onEventsMethodChange}>
                            <Radio value={EventsMethod.SEND_EVERYTHING} key={EventsMethod.SEND_EVERYTHING}>
                                <Text>{t('setup-webhook-pages_setup-webhook-content_tsx.Send', 'Send')} </Text>
                                <Text strong>{t('setup-webhook-pages_setup-webhook-content_tsx.everything', 'everything')}</Text>
                            </Radio>
                            <Radio value={EventsMethod.SELECT_INDIVIDUAL} key={EventsMethod.SELECT_INDIVIDUAL}>
                                {t('setup-webhook-pages_setup-webhook-content_tsx.Selectindividualeven', 'Select individual events')}
                            </Radio>
                        </Radio.Group>
                    </Form.Item>
                    {
                ...
                <Row justify='end'>
                    <Col>
                        <Button className='cvat-submit-webhook-button' type='primary' onClick={handleSubmit}>
                            {t('setup-webhook-pages_setup-webhook-content_tsx.Submit', 'Submit')}
                        </Button>
                    </Col>
                </Row>
            </Col>
