                    ...
                    >
                        {((): JSX.Element => {
                            if (activeInference.status === RQStatus.QUEUED) {
                                return (
                                    <>
                                        {t('tasks-page_automatic-annotation-progress_tsx.Automaticannotationr', 'Automatic annotation request queued')}
                                        <LoadingOutlined />
                                    </>
                                );
                            }
                            ...
                            if (activeInference.status === RQStatus.STARTED) {
                                return (
                                    <>
                                        {t('tasks-page_automatic-annotation-progress_tsx.Automaticannotationi', 'Automatic annotation is in progress')}
                                        <LoadingOutlined />
                                    </>
                                );
                            }
                            ...
                            if (activeInference.status === RQStatus.FAILED) {
                                return (<>{t('tasks-page_automatic-annotation-progress_tsx.Automaticannotationf', 'Automatic annotation failed')}</>);
                            }

                            if (activeInference.status === RQStatus.UNKNOWN) {
                                return (<>{t('tasks-page_automatic-annotation-progress_tsx.Unknownstatusreceive', 'Unknown status received')}</>);
                            }

                            return <>{t('tasks-page_automatic-annotation-progress_tsx.Automaticannotationa', 'Automatic annotation accomplisted')}</>;
                        })()}
                    </Text>
                </div>
                <Progress
