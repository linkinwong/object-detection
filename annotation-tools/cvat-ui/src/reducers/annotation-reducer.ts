    ...
    switch (action.type) {
        ...
        case AnnotationActionTypes.GET_JOB_SUCCESS: {
            ...
            if (job.dimension === DimensionType.DIMENSION_2D) {
                ...
                if (job.taskType === Workspace.AUDIO.toLocaleLowerCase()) {
                    workspaceSelected = Workspace.AUDIO;
                } else if (job.taskType === Workspace.TEXT.toLocaleLowerCase()) {
                    workspaceSelected = Workspace.TEXT;
                }
            } else {
        ...
        case AnnotationActionTypes.AI_TOOLS_AUTO_ANNOTATION: {
        ...
        case AnnotationActionTypes.AI_TOOLS_LOADING: {
            const { loading } = action.payload;
            return {
                ...state,
                isAIToolsLoading: loading,
            };
        }
        case AnnotationActionTypes.GET_AUDIO_INFO: {
            const { states } = action.payload;
            return {
                ...state,
                audio: {
