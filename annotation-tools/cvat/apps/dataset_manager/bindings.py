...
class CommonData(InstanceLabelData):
    ...
    def _init_frame_info(self):
        ...
        # elif hasattr(self._db_data, 'audio'):
        #     self._frame_info = {
        #         frame.id: {
        #             "path": "frame_{}".format(frame.path),
        #             "width": frame.width,
        #             "height": frame.height,
        #             "id": frame.id,
        #         } for frame in self._db_data.audio.all()
        #     }
        # elif hasattr(self._db_data, 'text'):
        #     self._frame_info = {
        #         frame.id: {
        #             "path": "frame_{:06d}".format(frame.path),
        #             "width": frame.width,
        #             "height": frame.height,
        #             "id": frame.id,
        #         } for frame in self._db_data.text.all()
        #     }
        else:
    ...
    def group_by_frame(self, include_empty: bool = False):
        ...
        if include_empty:
            # if hasattr(self._db_data, 'audio') or hasattr(self._db_data, 'text'):
            #     _t = set(self._frame_info)
            # else:
            #     _t = set(self._frame_info) & included_frames
            _t = set(self._frame_info) & included_frames
            for idx in sorted(_t):
                get_frame(idx)
