...
class IMediaReader(ABC):
    def __init__(self, source_path, step, start, stop, dimension, **kwargs):
    ...
    def _get_preview(obj):
        ...
        if mode == "I;16":
            ...
            preview = ImageOps.equalize(
                preview)  # The Images need equalization. High resolution with 16-bit but only small range that actually contains information
...
class IChunkWriter(ABC):
    ...
    @staticmethod
    def _compress_image(source_image: av.VideoFrame | io.IOBase | Image.Image, quality: int) -> tuple[
        int, int, io.BytesIO]:
        image = None
        ...
        if image.mode == "I;16":
            ...
            image = ImageOps.equalize(
                image)  # The Images need equalization. High resolution with 16-bit but only small range that actually contains information
...
class AudioReader(IMediaReader):
    def __init__(self, source_path, step=1, start=0, stop=None, dimension=DimensionType.DIM_2D, **kwargs):
        super().__init__(source_path=source_path, step=step, start=start, stop=stop, dimension=dimension, **kwargs)
        self.audio_segments = [AudioSegment.from_file(path) for path in source_path]
        self.total_frames = sum(len(audio.get_array_of_samples()) for audio in self.audio_segments)
        self.frame_rate = self.audio_segments[0].frame_rate if self.audio_segments else 0
        self.total_duration_seconds = sum(audio.duration_seconds for audio in self.audio_segments)

    # def __iter__(self):
    #     frame_num = 0
    #     for frame_num in range(self._start, self._stop, self._step):
    #         if self._has_frame(frame_num):
    #             yield self.get_frame(frame_num), self._source_path, frame_num
    ...
    #     start_ms = (frame_num / self.frame_rate) * 1000
    #     end_ms = start_ms + (1000 / self.frame_rate)
    #     return self.audio[start_ms:end_ms]

    # def get_progress(self, pos):
    #     return pos / self.total_frames
    ...
    # def get_preview(self, frame_num):
    #     start_ms = (frame_num / self.frame_rate) * 1000
    #     return self.audio[start_ms:start_ms+1000]  # 返回1秒的音频片段作为预览
    ...
    def get_frame(self, frame_num):
        ...
        for segment in self.audio_segments:
            if accumulated_frames + len(segment) > frame_num:
                start_ms = (frame_num - accumulated_frames) * 1000 / self.frame_rate
                end_ms = start_ms + 1000 / self.frame_rate
                return segment[start_ms:end_ms]
            accumulated_frames += len(segment)
        return None
    ...
    def get_duration(self):
        # return self.total_frames / self.frame_rate if self.frame_rate else 0
        return self.total_duration_seconds

    def get_preview(self, frame_num):
        frame = self.get_frame(frame_num)
        return frame[:1000]  # Return 1 second of audio as preview
...
def _is_text(path):
    ...
    return mime[0] is not None and any([mime[0].startswith(m) for m in mime_list])
...
#                         if packet.stream.metadata.get('rotate'):
#                             pts = image.pts
#                             image = av.AudioFrame().from_ndarray(
#                                 rotate_image(
#                                     image.to_ndarray(format='bgr24'),
#                                     360 - int(stream.metadata.get('rotate'))
#                                 ),
...
# class Mpeg3ChunkWriter(IChunkWriter):
#     FORMAT = 'mp3'
#     MAX_MBS_PER_FRAME = 36864
