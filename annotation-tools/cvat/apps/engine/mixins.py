...
from cvat.apps.engine.email import Email
...
class PartialUpdateModelMixin:
    ...
    def _update(self, request, *args, **kwargs):
        ...
        if response.status_code == 200 and response.status_text == 'OK':
            # 邮件通知逻辑
            assignee_id = request.data.get('assignee') or request.data.get('assignee_id')
            if assignee_id:
                assignee = User.objects.get(id=assignee_id)
                if assignee.email:
                    Email.send_assignee_email(assignee.email, request.data.get('assigneeUrl'))
        return response
