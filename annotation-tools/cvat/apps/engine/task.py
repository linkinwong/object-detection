...
from cvat.apps.engine.media_extractors import (MEDIA_TYPES, ImageListReader, Mpeg4ChunkWriter, Mpeg4CompressedChunkWriter,
    ...
    Mpeg3ChunkWriter, Mpeg3CompressedChunkWriter, AudioReader)
...
def create(db_task, data, request):
    ...
    with get_rq_lock_by_user(q, user_id):
        # _create_thread(db_task.pk, data)
        q.enqueue_call(
            func=_create_thread,
...
def _save_task_to_db(db_task: models.Task, *, job_file_mapping: Optional[JobFileMapping] = None):
    job = rq.get_current_job()
    job.meta['status'] = '任务已经保存到数据库'
    job.save_meta()

    if db_task.type in ['text', 'audio']:
        db_task.segment_size = 1
        db_task.overlap = 0
        db_segment = models.Segment()
        db_segment.task = db_task
        db_segment.start_frame = 0
        db_segment.stop_frame = 0
        db_segment.save()
...
def _validate_data(counter, manifest_files=None):
    ...
    for media_type, media_config in MEDIA_TYPES.items():
        if counter[media_type]:
            ...
            if manifest_files and media_type not in (
                'video', 'audio', 'image', 'zip', 'archive', 'text'
            ):
                raise Exception(
                    'File with meta information can only be uploaded with text/video/audio/images/archives'
                )
...
def _create_thread(
    ...
    *,
    ...
    if job_file_mapping is not None and task_mode != 'annotation':
        raise ValidationError("job_file_mapping 不能与视频等序列数据一起使用")
    ...
    if (not isBackupRestore and manifest_file and
        ...
        raise ValidationError("不能同时上传清单文件并使用随机排序")
    ...
    # slogger.glob.error(f"media: {media}")

    for media_type, media_files in media.items():
        if not media_files:
            continue

        if extractor is not None and task_type != 'text':
            slogger.glob.error(f'task_type: {task_type}')
    ...
    if (db_task.project is not None and
        ...
        raise ValidationError(
            f"任务的维度 ({validate_dimension.dimension}) 必须与项目中的其他任务 ({db_task.project.tasks.first().dimension}) 相同"
        )
    ...
    if validate_dimension.dimension != models.DimensionType.DIM_3D and (
        ...
        if job_file_mapping:
            ...
            if manifest is None:
                if not manifest_file or not os.path.isfile(os.path.join(manifest_root, manifest_file)):
                    raise FileNotFoundError(
                        "无法在上传的文件中找到上传清单文件 '{}'。使用“预定义”排序方法时，"
                        "此文件是输入文件中所必需的。"
                        "阅读更多: https://docs.cvat.ai/docs/manual/advanced/dataset_manifest/"
        ...
        for file_path in sorted_media_files:
            if not file_path in extractor:
                raise ValidationError(
                    f"在输入文件中找不到文件 '{os.path.basename(file_path)}'"
                )
    ...
    if db_task.type in [models.TaskType.AUDIO, models.TaskType.TEXT]:
        compressed_chunk_writer_class = Mpeg3CompressedChunkWriter
        original_chunk_writer_class = Mpeg3ChunkWriter
    ...
    # if task_type in new_type.keys():
    #     compressed_chunk_writer = None
    #     original_chunk_writer= None
    # else:
    ...
    if db_data.chunk_size is None:
        if  task_type=='audio':
            # 计算frames_per_chunk
            reader = AudioReader(video_path)
            duration_seconds = 10  # 每个chunk的持续时间（秒）
            db_data.chunk_size = int(reader.get_frame_rate() * duration_seconds)
        else:
    ...
    if settings.USE_CACHE and db_data.storage_method == models.StorageMethodChoice.CACHE:
        for media_type, media_files in media.items():
            ...
            if task_type in new_type.keys():
                db_images.extend([
                    new_type[task_type](data=db_data, path=path)
                    for path in media_files
                ])
            elif task_mode == MEDIA_TYPES['video']['mode']:
                try:
                    ...
                    if manifest_file:
                        try:
                            ...
                            _update_status('{} 开始准备有效的清单文件'.format(base_msg))

                    if not manifest_is_prepared:
                        _update_status('开始准备清单文件')
                    ...
                    base_msg = str(ex) if isinstance(ex, AssertionError) \
                        else "上传的视频不支持快速创建任务的方法"
                    _update_status("{} 任务将使用旧方法创建".format(base_msg))
            else:
    ...
    if db_task.type in new_type.keys():
        new_type[task_type].objects.bulk_create(db_images)
        db_images = []
    elif db_task.mode == 'annotation':
    ...
    #     if task_type not in new_type.keys():
    #         shutil.rmtree(upload_dir)

    # # Remove upload dir if it was created by share
    # if db_data.storage == models.StorageChoice.SHARE and data['server_files']:
    #     shutil.rmtree(upload_dir)

    # for path in validate_dimension.converted_files:
    #     os.remove(path)

    slogger.glob.info("Found frames {} for Data #{}".format(db_data.size, db_data.id))
    slogger.glob.info("Task #{} created".format(db_task.id))
