...
from cvat.apps.engine.models import (
    ...
    Comment, StorageMethodChoice, StorageChoice, AIAudioMateAnnotation,
    CloudProviderChoice, Location, CloudStorage as CloudStorageModel,
    Asset, AnnotationGuide, TaskType, DataChoice)
...
from cvat.apps.engine.serializers import (
    ...
    LabeledDataSerializer, AudioAnnotationsMetaSerializer,
...
from cvat.apps.engine.other_requests import AITools
...
class TaskViewSet(viewsets.GenericViewSet, mixins.ListModelMixin,
    ...
    def annotations(self, request, pk):
        # force call of check_object_permissions()
        self._object = self.get_object()
...
class JobViewSet(viewsets.GenericViewSet, mixins.ListModelMixin, mixins.CreateModelMixin,
    ...
    @action(detail=True, methods=['GET', 'DELETE', 'PUT', 'POST', 'OPTIONS'], url_path=r'audio_annotations/?$')
    def audio_annotations(self, request, pk):
        job = get_object_or_404(Job, pk=pk)
        ans = AIAudioMateAnnotation.objects.filter(job=job)
        annotations = AudioAnnotationsMetaSerializer(ans, many=True)
        if request.method == 'GET':
            ...
            data = request.data.get('annotations', [])
            ...
            for row in data:
                row['job'] = pk
                serializer = AudioAnnotationsMetaSerializer(data=row)
                if serializer.is_valid():
                    instances.append(serializer.validated_data)
                else:
                    print(serializer.errors)

            if instances:
                ans.delete()
                AIAudioMateAnnotation.objects.bulk_create([
                    AIAudioMateAnnotation(**instance) for instance in instances
                ])
                return Response(status=status.HTTP_201_CREATED)
            else:
    ...
    def data(self, request, pk):
        ...
        # if the job is a video job, we need to return the video file
        if db_job.segment.task.type == DataChoice.AUDIO:
            t_id = str(db_job.segment.task.data_id)
            # todo: optimize the hard code
            file_path: str = os.path.join(settings.MEDIA_DATA_ROOT, t_id, 'raw')
            if os.path.exists(file_path):
                res = Response(data='No data found', status=status.HTTP_404_NOT_FOUND)
                for root, dirs, files in os.walk(file_path):
                    if len(files):
                        # loop files and pick the first audio file
                        for file in files:
                            # wav or mp3
                            if file.endswith(tuple(['.mp3', '.wav'])):
                                res = Response({
                                    "url": os.path.join(settings.MEDIA_PATH, t_id, 'raw', file),
                                    'filename': file,
                                    'ret': True,
                                    "feedback": "",
    ...
    def AITools(self, request, pk):
        ai_type_list = ['ch', 'en', 'face', 'stereo']
        db_job = self.get_object()  # call check_object_permissions as well
        ai_type = request.query_params.get('type', None)
        task_id = str(db_job.segment.task_id)
        filename = request.query_params.get('frameFilename', None)
        user_id = str(request.user.id)

        path = os.path.join(BASE_DIR, 'data', 'data', task_id, 'raw', filename)

        headers = {
            'X-Request-Id': "_".join([user_id, task_id, pk]),
            'X-User-Id': user_id,
        }

        if ai_type in ['ch', 'en']:
            ai_tools_data = AITools.ocr(path, ai_type, headers)
        else:
            ai_tools_func = getattr(AITools, ai_type)
            ai_tools_data = ai_tools_func(path, headers)

        return Response({'ret': True, 'data': ai_tools_data})
    ...
    def metadata(self, request, pk):
        ...
        if db_job.segment.task.type in ['text', 'audio'] and not frame_meta:
            frame_meta = [{
                'width': 200,
                'height': 200,
                'name': 'audio.png',
